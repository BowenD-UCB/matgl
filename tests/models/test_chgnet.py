...
class TestCHGNetPotential:
    ...
    def test_threebody_forces_finite_difference(self):
        ...
        ff = Potential(model=model, debug_mode=True)
        g_m, _, state = p2g.get_graph(struct_m)
        g_0, _, _ = p2g.get_graph(struct_0)
        g_p, _, _ = p2g.get_graph(struct_p)

        lat_t = torch.tensor(struct_0.lattice.matrix, dtype=matgl.float_th)
        e_m, _, _ = ff(g_m, lat_t, state)
        _, grad_zero, _ = ff(g_0, lat_t, state)
...
_MATPES_EXPECTED = {
    ...
    ("r2SCAN", "fe"): {
        ...
        "forces": [
            [-0.0, -3.725290298461914e-08, -2.9802322387695312e-08],
            [-3.073364496231079e-08, 3.67872416973114e-08, 1.4901161193847656e-08],
        ],
        "stress": [
            [0.9716099500656128, 3.668358772301872e-07, -4.347684523509088e-07],
            [2.98903330531175e-07, 0.9716094732284546, -1.449228221872545e-07],
            [-2.8078795821784297e-07, -3.577782194952306e-07, 0.9716099500656128],
        ],
    ...
    ("PBE", "fe"): {
        ...
        "forces": [
            [-2.9802322387695312e-08, -9.313225746154785e-08, -9.313225746154785e-08],
            [4.086177796125412e-08, 9.301584213972092e-08, 9.313225746154785e-08],
        ],
        "stress": [
            [7.741415023803711, 5.445928081826423e-07, 1.8115352418135444e-07],
            [6.895156161590421e-07, 7.741415500640869, -2.5361492816955433e-07],
            [1.8228573139822402e-07, -3.509849477723037e-08, 7.741417407989502],
        ],
